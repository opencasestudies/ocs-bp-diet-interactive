...
```{r, eval=FALSE}
diet_data <- readr::read_csv(here("OCS_data", "data", "raw", 
                       "dietary_risk_exposure_all_ages_2017.csv"))
sep_age_diet_data <- read_csv(here("OCS_data", "data", "raw", 
                       "dietary_risk_exposure_sep_ages_2017.csv"))
```
...
Let's see if the `location_name` values are the same between both CSV files. To do this we will use the `setequal()` function of `dplyr`. The `setequal()` function allows us to check if there's a difference between 2 tibbles. The `setequal()` function requires that column names match in order for it to check if there is a difference. 
```{r}
dplyr::setequal(
...
load(here::here("OCS_data", "data", "imported", "imported_data.rda"))
```
...
In this case we are also using a combination of the assignment operator and the `%>%` pipe. This allows us use the `pdf_table` data as input to the later steps but also reassign the output to the same data object name.
...
```{r}
class(category)
category <- category %>% unlist()
amount <- amount %>% unlist()
class(category)
```
...
If we wanted to remove the direction variable we could use the `mutate()` function of the `dplyr` package:

```{r,eval = FALSE}
guidelines %>% mutate(direction = NULL)
```
...
sep_age_diet_data <- sep_age_diet_data %>% 
```
...
`setequal()` does not provide clues about what is different but TRUE (no differences) or FALSE (indicating at least one difference). Remember that the `setequal()` function requires that column names match in order for it to check if there is a difference. 
...
# Note: setequal() requires the same column names to compare. 
consumption <- distinct(diet_data, dietary_risk) %>% 
  rename(food = dietary_risk)
...
guidelines <- guidelines %>% 
  mutate(optimal = if_else(condition = unit_optimal == "mg",
    true = as.numeric(optimal) / 1000,
    false = as.numeric(optimal)
  ))
```
...
guidelines <- guidelines %>% 
  mutate(upper_optimal = if_else(condition = unit_optimal == "mg",
    true = as.numeric(upper_optimal) / 1000,
    false = as.numeric(upper_optimal)
  ))
```
...
guidelines <- guidelines %>% 
  mutate(lower_optimal = case_when(
    unit_optimal == "mg" ~ as.numeric(lower_optimal) / 1000,
    unit_optimal != "mg" ~ as.numeric(lower_optimal)
  ))
...
```{r}
diet_and_guidelines <- diet_and_guidelines %>% 
  mutate(Relative_Percent = (mean / optimal) * 100)

all_age_diet_and_guidelines <- all_age_diet_and_guidelines %>% 
  mutate(Relative_Percent = (mean / optimal) * 100)
```
...
quiz(caption = "",
  question("Which one of the pipe operators should be used right after a variable name if we want to perform a sequence of operations on that variable?",
    answer("`%>%`", correct=TRUE),
    answer("`%<%`", message = "This is not a valid pipe operator."),
...
load(here::here("OCS_data", "data", "wrangled", "wrangled_data.rda"))
```
...
```{r, echo = FALSE, out.width="400ptx"}
set.seed(13)
thedata <- thedata %>%  mutate(y2 = rnorm(20, sd = 40))
